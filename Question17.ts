...
let guestList: string[] = ["Rohan", "Talal", "Waleed"];
let message: string = ("Sorry, the dinner table can only accommodate two guests.");
console.log("Origanal Guest List:", guestList);

while (guestList.length > 2) {
    var removedGuest = guestList.pop();
    if (removedGuest) {
        console.log(`${message}. Sorry, ${removedGuest},I can't invite you to dinner.`);
    }
}

guestList.forEach(guest => {
    console.log(`Dear ${guest}, You are still invite to dinner`);
});
